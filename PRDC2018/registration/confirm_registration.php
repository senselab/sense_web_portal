	...
	<button id="downloadPendingCsv" class="btn btn-primary">
		...
		<?php
			...
			if ( !allow_to_visit($_SERVER['REMOTE_ADDR']) ){
				header("Location: " . $prdc . "/registration2018.php?err=&msg=You have no permission to visit the page!");
			...
			$conn = dbconnect();
			$sql_cc_confirmed = "SELECT * FROM `orders` WHERE `payment_confirmed`=1 and `invalid`=0 ORDER BY CONVERT(`paper_id`, unsigned INTEGER) ASC, `created_at` ASC";
			$sql_cc_unconfirmed = "SELECT * FROM `orders` WHERE `payment_confirmed`=0 and `invalid`=0 ORDER BY CONVERT(`paper_id`, unsigned INTEGER) ASC, `created_at` ASC";
...
EOF;
			...
			foreach($confirmed_orders as $__idx=>$order){
				...
				<p><tr>
					...
					<td><a href="$prdc/registration2018.php?reftoken=$reftoken">#$reftoken</td>
					<td>$reg_type</td>
					...
					<td>$trans_id</td>
					<td>$trans_time</td>
					<td style="display: none">$mealtype</td>
					<td>
...
<script type="text/javascript">
	var keys = ["#", "Title", "Name", "Country", "Affiliation", "Email", "Order num", "Reg Type", "Member Num", "Paper ID", "Paper Title", "Banquet#", "Amount", "Payment Method", "Status", "Create Time", "Transaction ID", "Transaction Time", "Mealtype"];
	var confirmedCsv = [];
	...
	$('#cc_confirmed_table tr').each(function() {
		...
		if (fields.length > 0){
			fields.slice(0, 19).each(function(){ entry.push($(this).text()); });
